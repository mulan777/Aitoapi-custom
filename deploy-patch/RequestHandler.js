...
class RequestHandler {
    ...
    _buildProxyRequest(req, requestId) {
        ...
        if (req.method === "POST" && bodyObj) {
            if (bodyObj.contents) {
                const removedTrailingTurns = FormatConverter.removeUnsupportedTrailingModelTurns(
                    bodyObj.contents,
                    cleanModelName
                );
                if (removedTrailingTurns > 0) {
                    this.logger.warn(
                        `[Proxy] ${cleanModelName} does not support prefilling; removed ${removedTrailingTurns} trailing model turn(s).`
                    );
                }
